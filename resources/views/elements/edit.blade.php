...
<div class="form-page">
    <div class="form-card">
        ...
        <div class="form-card-body">
            ...
            <form method="POST" action="{{ route('elements.update', ['category' => $category->id, 'element' => $element->id]) }}" class="form">
                @csrf
                @method('PUT')
                
                <div class="form-group">
                    <label for="nom" class="form-label">Nom</label>
                    <input type="text" class="form-input @error('nom') input-error @enderror" 
                           id="nom" name="nom" value="{{ old('nom', $element->nom) }}" required>
                    @error('nom')
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-input @error('description') input-error @enderror" 
                              id="description" name="description" rows="3">{{ old('description', $element->description) }}</textarea>
                    @error('description')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="media-section" style="margin-top: 2rem; border-top: 1px solid #eee; padding-top: 2rem;">
                ...
                <form method="POST" action="{{ route('elements.upload', ['category' => $category->id, 'element' => $element->id]) }}" 
                ...
                @if($element->multimedias->isNotEmpty())
                    <div class="media-grid" style="margin-top: 1rem; display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 1rem;">
                        @foreach($element->multimedias as $media)
                            <div class="media-card" style="border: 1px solid #eee; border-radius: 8px; overflow: hidden;">
                                ...
                                @elseif(Str::endsWith(strtolower($media->chemin), ['.mp3', '.wav', '.ogg']))
                                    <div style="width: 100%; height: 150px; background: #f0f0f0; display: flex; align-items: center; justify-content: center; flex-direction: column;">
                                ...
                                <div style="padding: 0.5rem;">
                                    <form action="{{ route('multimedias.destroy', ['category' => $category->id, 'multimedia' => $media->id]) }}" 
                                          method="POST" class="inline-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="button button-small button-danger" style="width: 100%;"
